...
public class ModificarDocente extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
            throws ServletException, IOException {
        String buscando = request.getParameter("Buscar");
        NegocioDocente negD = new NegocioDocente();
        Docente d = new Docente();

        d.setRut(request.getParameter("rut"));
        d.setNombre(request.getParameter("nombres"));
        d.setFnac("0000-00-00");
        d.setSexo(request.getParameter("sexo"));
        d.setDireccion(request.getParameter("direccion"));
        d.setComuna(request.getParameter("comuna"));
        d.setEmail(request.getParameter("email"));
        d.setPass(request.getParameter("rut").substring(2, 8));
        d.setTelefono(request.getParameter("telefono"));
        d.setEsJefe(true);
        d.setCursoJefe(request.getParameter("cursoJefe"));

        negD.modificarDocente(d,buscando);
        System.out.println("a buscar:" +buscando);
                RequestDispatcher envio = request.getRequestDispatcher("SucesoModificarDocente.jsp");
